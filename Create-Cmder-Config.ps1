function write_config_comments($json, $var) {
    ...
    write_line_to_file $var $var.bashConfig "#!/bin/bash/env bash"
    write_line_to_file $var $var.bashConfig "# $msg"

    write_line_to_file $var $var.psConfig "# $msg"
...
function write_config_paths($json, $var) {
    $json.paths | ForEach-Object -Process {
        ...
        write_line_to_file $var $var.bashConfig "export PATH=\$((Convert-Path $absolutePathToBinary)):`$PATH".Replace(":\", "\").Replace("\", "/")
        write_line_to_file $var $var.psConfig "`$env:Path = `"${absolutePathToBinary};`" + `$env:Path"
        write_line_to_file $var $var.cmdConfig "set PATH=${absolutePathToBinary};%PATH%"
