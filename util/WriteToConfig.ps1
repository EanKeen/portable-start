## Writes to a certain config file ##
function write_to_config($var, $configFile, $content) {
  ...
  elseif(($configFile -ne $var.bashConfig) -and ($configFile -ne $var.psConfig) -and ($configFile -ne $var.cmdConfig)) {
    print_warning "write_variable_to_config" "Cannot perform operation on `"$configFile`" because it does not recognize the file."
    return
  }
...
function write_path_to_config($var, $configFile, $binName, $filePath) {
  ...
  elseif($configFile -eq $var.cmdConfig) {
    ...
    print_warning "write_path_to_config" "Cannot perform operation on `"$configFile`" because it does not recognize the file."
    return
  }
...
function write_alias_to_config($var, $configFile, $aliasName, $aliasValue) {
  ...
  elseif($configFile -eq $var.cmdConfig) {
    print_warning "write_alias_to_config" "Does not support writing an alias to `$cmdConfig"
  }
  else {
    print_warning "write_alias_to_config" "Cannot perform operation on `"$configFile`" because it does not recognize the file."
    return
  }
